import { ReducersMapObject, combineReducers, Action } from "redux";
...
const stacksReducers: ReducersMapObject = {
    stack: (state, action) => {
        if (action.type == TRCreateActions.CREATE_GOBACK
            || action.type == TRListActions.DETAIL_GOBACK) {
            let all: any[] = [...state.routes];
            all.pop();
            return { ...state, index: state.index - 1, routes: all }
        }
...
export const reducers = (state: GlobalState, action: Action) => {
    let res = <GlobalState>combine(state, action);
    //ROUTES
    res.header = { ...res.header, title: "", rightIcon: "", leftIcon: "", searchTransaction: false, searchContact: false };
